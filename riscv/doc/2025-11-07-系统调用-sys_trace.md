...
提供了一个用户级程序版本的trace，它运行另一个启用了跟踪的程序（参见*user/trace.c*）。完成后，您应该看到如下输出：
```bash
$ trace 32 grep hello README		# 
3: syscall read -> 1023				# 打印： 进程ID: 系统调用名称 -> 系统调用返回值
3: syscall read -> 966
3: syscall read -> 70
3: syscall read -> 0
```
...
## trace系统调用的工作原理
1. 语法：trace mask command [args]

	- mask：一个位掩码，用于指定要跟踪哪些系统调用
	- command：要执行的命令
	- args：命令的参数（可选）
2. 位掩码：

	- 每个系统调用都有一个唯一的编号（在kernel/syscall.h中定义）
	- 掩码中的每一位对应一个系统调用
	- 如果某一位为1，则跟踪对应的系统调用；为0则不跟踪
3. 输出格式：

	- pid: syscall name -> return_value
	- pid：执行系统调用的进程ID
	- syscall name：系统调用的名称
	- return_value：系统调用的返回值
4. 示例解释
$ trace 32 grep hello README
	- 32是掩码值，二进制为00100000
	- 这意味着只跟踪第6位对应的系统调用（从0开始计数）
	- 在xv6中，系统调用编号6对应read系统调用
	- 因此，这个命令会跟踪grep程序执行的所有read系统调用
